...
const SpeakerCard = ({ speaker, bgIndex }) => {
  ...
  return (
    <div className="laptop:z-50 h-[600px] laptop:h-[390px] p-4 rounded-lg flex flex-col laptop:flex-row items-stretch">
      <div
        className={`w-full laptop:w-1/3 p-2 rounded-lg shadow-lg flex flex-col items-center justify-between text-center m-2 transition-all duration-700 ${bgColors[bgIndex]} ${textColors[bgIndex]}`}
      >
        <div className="flex-grow flex items-center justify-center">
          <img
            src={speaker.photo}
            alt={speaker.name}
            className="w-40 h-40 laptop:w-60 laptop:h-60 p-1 object-cover"
          />
        </div>
      ...
      >
        <div className="flex-grow mb-2 laptop:mb-1 h-auto laptop:text-sm">
          <p>{speaker.description}</p>
        </div>

        <div className="flex justify-end items-end w-full scale-[0.8]">
          <button
            className={`border ${borderColors[bgIndex]} ${textColors[bgIndex]} px-2 py-2 cursor-pointer rounded-full flex items-center gap-1 transition duration-300
